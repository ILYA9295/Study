...
Представьте, что
перед вами стоит задача переписать его так, чтобы все функции 
стали методами объекта personalMovieDB
...
свойство privat. Если оно false - он
переключает его в true, если true - переключает в false. 
Протестировать вместе с showMyDB.

3) В методе writeYourGenres запретить пользователю нажать 
кнопку "отмена" или оставлять пустую строку. 
...
while (personalMovieDB == ''  || personalMovieDB.count == null || isNaN
...
writeYourGenres: function(){
    for (let i = 1; i < 2; i++) {  //в первом способе i<=3
...
//добавдяем перевод всего в нижний регистр для верной сортировки 
         if (genres === '' || genres == null) {
            console.log('not correct');
             i--;
        } else {
